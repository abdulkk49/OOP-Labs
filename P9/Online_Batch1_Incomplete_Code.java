		...
		synchronized (gameData.lock1) {
			...
			while (!(gameData.playerSuccessFlag[0] || gameData.playerSuccessFlag[1])) {
				...
				synchronized (gameData.lock2) {
					try {
						while(this.numberAnnounced == 0) gameData.lock2.wait();
				...
				// the numbers 						
				try {
					while(!gameData.playerChanceFlag[0] || !gameData.playerChanceFlag[1]) gameData.lock1.wait();
				} catch(InterruptedException e) {
	...
	public void run() {
		...
		synchronized(gameData.lock1){			
			...
			while (!gameData.gameCompleteFlag) {
				...
				try {
					while (!gameData.noAnnouncedFlag || gameData.playerChanceFlag[id]) gameData.lock1.wait();
				} catch (InterruptedException e) {
