...
class Dealer implements Runnable {
	...
	public void run() {
		...
		synchronized(gameData.lock1){
			...
			while(!(gameData.playerSuccessFlag[0] == true || gameData.playerSuccessFlag[1] == true)){
...
class Player implements Runnable {
	...
	public void run() {
		...
		synchronized(gameData.lock1){			
			...
			/* STEP-13: Specify condition */while(!gameData.gameCompleteFlag) {
				...
				if(gameData.noAnnouncedFlag == true || gameData.playerChanceFlag[id] == false){
					try{
						gameData.lock1.wait();
					}catch(InterruptedException e){}
				}
